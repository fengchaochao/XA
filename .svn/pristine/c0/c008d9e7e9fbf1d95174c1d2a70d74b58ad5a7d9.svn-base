...
<head>
    ...
    #commodityDescription {width: 723px;max-width: 725px}
    </style>
</head>
...
<body class="mainContant">
	...
	<input type="hidden" id="id" value="${goods.id}"/>
	    <div class="form-group rowshortBox" style="margin-top: 15px" >
			...
			<div class="col-sm-10">
			    <input type="text" class="form-control" id="commodityName" name="commodityName"  value="${goods.commodityName}"/>
			</div>
		</div>
	    <div class="form-group rowshortBox" style="margin-top: 15px" >
        ...
        <div class="form-group rowshortBox">
            ...
            <div class="col-sm-10">
                <input type="text" class="form-control" id="promotionFee" name="promotionFee"  value="${goods.promotionFee}"/>
            </div>
        </div>
        <div class="form-group rowshortBox">
            <label class="control-label col-sm-2">配送费</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" id="shippingFee" name="shippingFee" value="${goods.shippingFee}"/>
            </div>
        </div>
                ...
                <div class="tabInform">
                    ...
                    <c:forEach items="${goodsSpecificationsList}" var="var" varStatus="indx">
                        <p class="specifications">
                            <input type="text" class="" id="specificationsName${indx.index+1}" value="${var.specificationsName }"/>
                            <input type="text" class="" id="inventory${indx.index+1}" value="${var.inventory }"/>
                            <input type="text" class="" id="price${indx.index+1}" value="${var.price}"/>
                            <button class="redText" <c:if test="${indx.index!=0}"> class="delete" </c:if>>删除</button>
                        </p>
                    </c:forEach>
                </div>
            </div>
		...
		<div class="form-group" style="margin-top: 15px" >
			...
			<div class="col-sm-10">
			    <textarea id="commodityDescription" name="commodityDescription" rows="10">${goods.commodityDescription}</textarea>
			</div>
		</div>
		<div class="form-group">
            <label class="control-label col-sm-2">商品详情图片</label>
            <div class="col-sm-10" style="width:765px;">
                <input type="hidden" disabled="disabled" id="goodsPhotos" class="form-control" value="${goods.goodsPhotos}"/>
                <div id="material1">
                    <c:forEach items="${goodsPhotos}" var="var">
                        <div style="display:inline-block;margin:5px;">
                            <span class="imgDelet1" id="${var}" style="display:block;cursor: pointer;color:#f74f4c;">删除</span>
                            <img alt="" id="licencePicture_img1" src="<spring:url value='${var}' />"  class="pic" style="width: 130px;height: 90px"/>
                        </div>
                    </c:forEach>
                </div>
			...
			<div class="col-sm-10" style="width:765px;">
				...
				<div id="material">
                    <c:forEach items="${commodityImages}" var="var">
                        <div style="display:inline-block;margin:5px;">
                            <span class="imgDelet" id="${var}" style="display:block;cursor: pointer;color:#f74f4c;">删除</span>
