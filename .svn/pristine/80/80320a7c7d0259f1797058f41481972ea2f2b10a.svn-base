...
<body class="mainContant">
    ...
    <div class="pageHeadline">
       <span class="leftNotice">商家锁定的用户详情</span>
    </div>
   <!--end pageHeadline -->
    <div class="whiteBox">
	    <form id="formSearch" class="form-horizontal">
			<div class="form-group" style="margin-top: 15px;margin-left: -31px;">
			<input type="hidden" id="businessInformationId" value="${businessInformationId}"/>
				<label class="control-label" for="userType" style="float: left;margin-left: 3%;">账户类别</label>
				<div class="col-sm-2">
					<select name="merchantsState" id = "userType" class="form-control" >
	...
	<script type="text/javascript">
		...
		function initTable() {
            $table = $('#tb_board').bootstrapTable({
                method: 'get',
                url : '<spring:url value="/headquarters/consumersAccount/doReadBusinessLocalUserList" />?businessInformationId='+$("#businessInformationId").val(),
                pagination: true,
                pageSize: 10,
		...
		// 编辑操作
        function editRecord() {
        	window.location.href='<spring:url value="/headquarters/businessInformation/doEnBusinessInformationEdit"/>'+'?businessInformationId='+$("#businessInformationId").val();
        }
	</script>
