...
public class FileUploadController extends BaseAction {
	 ...
	 * 轮播图
	 * */
	@RequestMapping(value = "/uploadifyFigure", method = RequestMethod.POST)
	@ResponseBody
	public RespBean<Map<String,String>> uploadifyFigure(MultipartFile upufdmfile) throws Exception
	{
		Map<String,String> respMap = new HashMap<String,String>();
		...
		if (!upufdmfile.isEmpty()) {
			...
			String projectPath= doGetRequst().getSession().getServletContext().getRealPath("/");
			// 文件路径
			String filePath = "/ShufflingFigure" + getFilePath();
			// 绝对路径
			String absolutePath = projectPath + filePath;
			File fold = new File(absolutePath);
			if(!fold.exists()){
			...
			String relativePath = filePath + "/" + newFileName + "." + suffix;
			//获取图片的宽和高
			MultipartFile file1 = upufdmfile; 
	        CommonsMultipartFile cf= (CommonsMultipartFile)file1; 
	        DiskFileItem fi = (DiskFileItem)cf.getFileItem(); 
	        File picture = fi.getStoreLocation();
	...
	public void imageUpload(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			CkeditorUpload.ckeditor(request, response, DirectoryName);
