   ...
   //输入验证
	...
	//取消
   	$("#cancel").bind("click",function(){
		window.location.href='<spring:url value="/headquarters/memberStatistics/doEnAgentEdit"/>'+'?agentId='+$("#id").val();
		...
		$.ajax({
			...
			success: function(data){
				if(data){
					...
					window.location.href='<spring:url value="/headquarters/memberStatistics/doEnAgentEdit"/>'+'?agentId='+id;
				}else{
...
function bussinessView(id){
	window.location.href='<spring:url value="/headquarters/businessInformation/doEnBusinessInformationEdit"/>'+'?businessInformationId='+id+'&flag=5';
}
function consumersView(id){
	window.location.href='<spring:url value="/headquarters/consumersMarketers/doEnConsumersEdit"/>'+'?consumersId='+id+'&flag=3';
}
</script>
</html>
