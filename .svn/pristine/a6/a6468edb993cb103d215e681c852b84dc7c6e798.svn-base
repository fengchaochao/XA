	...
	<script type="text/javascript">
		...
		$(function() {
            ...
            $('#btn_add').click(function () {
            	parent.layer.open({
    			    ...
    			    fix: false, //不固定
    			    maxmin: true,
    			    content: '<spring:url value="/messageObject/doEnMessageObjectEdit" />'
    			});
            });
		});
		...
		function initTable() {
            $table = $('#tb_board').bootstrapTable({
                method: 'get',
                url : '<spring:url value="/messageObject/doReadMessageObjectList" />',
                pagination: true,
                pageSize: 10,
        ...
        function editRecord(index) {
			...
			parent.layer.open({
			    ...
			    fix: false, //不固定
			    maxmin: true,
			    content: '<spring:url value="/messageObject/doEnMessageObjectEdit"/>'+'?messageObjectId='+row.id
			});
        }
