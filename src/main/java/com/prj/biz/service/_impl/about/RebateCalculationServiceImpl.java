...
public class RebateCalculationServiceImpl implements RebateCalculationService{
	...
	public void transferRecord(Order order) throws Exception{
		...
		transferRecord.setMoney(String.valueOf(moneys));
		transferRecord.setCreattime(UfdmDateUtil.getCurrentTime());
		transferRecord.setBusinessId(order.getBusinessId());
		transferRecord.setOrderNumber(order.getOrderNumber());
		
		SysUser sysUser = new SysUser();
		sysUser.setMerchantsId(order.getBusinessId());
		...
		transferRecordService.doSave(transferRecord);
	}
	...
	public void lockedDraw(String orderId, String payAccount,
		...
		if (consumersAccounts.size() > 0) {
			...
			if(StringUtils.isEmpty(order.getPurchaserId())){
				consumersAccount.setConsumersId(order.getPurchaserId());
			}
