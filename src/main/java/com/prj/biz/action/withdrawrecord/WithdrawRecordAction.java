...
import com.prj.utils.ping.IdentificationExample;
...
public class WithdrawRecordAction extends BaseAction
{
	...
	public RespBean<String> doSaveBankInfo(BankWithdrawal bankWithdrawal) throws Exception{
		RespBean<String> respBean = new RespBean<String>();
		SysUser sysUsers = (SysUser) (SecurityUtils.getSubject().getSession().getAttribute(SysConstants.SESSION_SYS_USER));
		IdentificationExample eventExample = new IdentificationExample();
		String msg=eventExample.identifyBankCard(bankWithdrawal.getBrankName(), bankWithdrawal.getBrankNumber(), bankWithdrawal.getIdCard());
		if("1".equals(msg)){
			respBean.setBody("银行卡信息认证失败");
		}
		if("0".equals(msg)){
			bankWithdrawal.setUserId(sysUsers.getId());
			bankWithdrawal.setAccountType("3");
