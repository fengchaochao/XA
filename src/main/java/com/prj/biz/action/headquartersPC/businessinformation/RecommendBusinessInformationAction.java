...
public class RecommendBusinessInformationAction extends BaseAction {
						...
						if(codeAreas.get(i)!=null){
							...
							if(businessInformationList.size()>0){
								for (int j = 0; j < businessInformationList.size(); j++) {
									...
									SysUser sysUser=sysUserService.doGetById(businessInformationList.get(j).getFounder());
									BusinessInformation businessInformation2=businessInformationService.doGetById(sysUser.getMerchantsId());
									if(businessInformation2!=null){
										businessInformationList.get(j).setFounderName(businessInformation2.getVendorName());
										businessInformationList.get(j).setConsumerNo(businessInformation2.getMerchantNumber());
										businessInformationList.get(j).setMerchantPhone(businessInformation2.getMerchantPhone());
									}else{
										Consumers consumers=consumersService.doGetById(sysUser.getMerchantsId());
										if(consumers!=null){
											businessInformationList.get(j).setFounderName(consumers.getNickName());
											businessInformationList.get(j).setConsumerNo(consumers.getCustomerNumber());
											businessInformationList.get(j).setMerchantPhone(consumers.getPhone());
										}
									}
	...
	public ModelAndView doEnBusinessInformationEdit(String businessInformationId,String flag)
		...
		SysUser sysUsers = (SysUser) (SecurityUtils.getSubject().getSession().getAttribute(SysConstants.SESSION_SYS_USER));
		if("1".equals(sysUsers.getUserState())){
			agent=agentService.doGetById(sysUsers.getMerchantsId());
		}
			...
			if(agentsList.size()>0){
				...
				if("1".equals(msg)){
					...
					if ("/".equals(File.separator)) {
						imgs = projectPath + "/img/login/'"
								+ businessInformation.getId() + "'.png";
					...
					MatrixToImageWriter.createQRcode(PayConfiger.project_path+ "/api/goH5Pay?storeId="
									+ businessInformation.getId() + "-S",qrCodePAth, "", "", "");

					businessInformation.setQrCode(PayConfiger.project_path + "/qrcode/"+businessInformation.getId() +".jpg");
					...
					sysUser.setRealName(businessInformation.getMerchantPhone());
