...
<body class="mainContant">
   ...
   <div class="pageHeadline">
      <span class="leftNotice">提现管理</span>
      <span class="greenBtn" id="btn_add">提现记录</span>
   </div>
   <!--end pageHeadline -->
	...
	<script type="text/javascript">
		...
		$(function() {
            ...
            $('#btn_add').click(function () {
    			window.location.href='<spring:url value="/applyCheck/doEnApplyCheckedList" />';
            }); 
		...
		function initTable() {
            $table = $('#tb_board').bootstrapTable({
                method: 'get',
                url : '<spring:url value="/applyCheck/doReadApplyCheckList" />',
                pagination: true,
                pageSize: 10,
		...
		// 同意提现
        function agreeRecord(index) {
        	var row=$table.bootstrapTable('getData')[index];
        	$.ajax({
					type: "POST",
					url : '<spring:url value="/applyCheck/doEnAgreeWithdraw"/>',
					data:{
						id:row.id
					},
					dataType: "json",
        ...
        // 拒绝提现
        function unAgreeRecord(index) {
        	var row=$table.bootstrapTable('getData')[index];
        	layer.confirm('确认驳回该条提现申请？', {
			  btn: ['确认','取消'] //按钮
			  }, function(){
				$.ajax({
					type: "POST",
					url : '<spring:url value="/applyCheck/doEnUnAgreeWithdraw"/>',
					data:{
						id:row.id
					},
					dataType: "json",
					...
					success: function(data){
						 if("0000000"==data.head.respCode){
							window.location.href='<spring:url value="/applyCheck/doEnApplyCheckList"/>';	
							layer.msg('操作成功', {icon: 1});
						}
						else{
							layer.msg('操作失败', {icon: 6});
        ...
        function operateFormatter(value, row, index) {
            var e = '<button type="button" class="greenText" style="width:70px;" onclick="agreeRecord(\''+ index + '\')">同意提现</button> <button type="button" style="width:70px;"  class="redText" onclick="unAgreeRecord(\''+ index + '\')">拒绝提现</button>';  
            return e;  
        }
