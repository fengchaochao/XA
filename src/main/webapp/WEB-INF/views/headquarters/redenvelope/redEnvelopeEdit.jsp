...
<link
	href='<spring:url value="/css/plugin/bootstrap-table/bootstrap-table.min.1.10.1.css" />'
	rel="stylesheet" />
<style>
	...
	<form id="submitForm" class="form-horizontal">
	    <div class="whiteBox">
	       <div class="form-group" style="margin-top: 15px">
	            ...
	            <div class="col-sm-5">
	                <input type="text" class="form-control" id="totalPrice" name="totalPrice" value="${redEnvelope.totalPrice}" disabled="disabled"/>
	            </div>
	        </div>
	        ...
	        <div class="form-group">
	            ...
	            <div class="col-sm-5">
	                <input type="text" class="form-control" id="number" name="number" value="${redEnvelope.number}" disabled="disabled"/>
	            </div>
	        </div>
	        <div class="form-group">
	            ...
	            <div class="col-sm-5">
	                <input type="text" class="form-control" id="createDate"
	                    onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d'})" value="${redEnvelope.createDate}" disabled="disabled"/>
	                <em>-</em>
	                <input type="text" class="form-control" id="createTime"
	                    onclick="WdatePicker({dateFmt:'HH:mm:ss',minDate:'%H:{%m+15}:%s'})" value="${redEnvelope.createTime}" disabled="disabled"/>
	            </div>
	        </div>
	        <div class="form-group">
	            ...
	            <div class="col-sm-5">
	                <input type="text" class="form-control" id="expirationDate" name ='expirationDate'
	                     value="${redEnvelope.expirationDate}" disabled="disabled"/>
	            </div>
	        </div>
	        <div class="form-group">
	            <label class="control-label col-sm-2">红包种类</label>
	            <div class="col-sm-10" style="line-height:32px;">
	                <input type="radio" name="redEnvelopeType" id="redEnvelopeType" value="0" <c:if test="${redEnvelope.redEnvelopeType=='0'}"> checked="checked" </c:if> onclick="random();" disabled="disabled"/>随机红包
	                <input type="radio" name="redEnvelopeType" id="redEnvelopeType1" value="1" <c:if test="${redEnvelope.redEnvelopeType=='1'}"> checked="checked" </c:if> onclick="fixedAllocation();" disabled="disabled" style="margin-left:20px;" />固定红包
	            </div>
	        </div>
	        <c:if test="${redEnvelope.redEnvelopeType=='0'}">
	            <div class="form-group" id="randomPrice">
	                ...
	                <div class="col-sm-3" style="width:20%;">
	                    <input type="text" class="form-control" id="maxPrice" value="${redEnvelope.minPrice}" disabled="disabled"/>
	                </div>
	            </div>
	        </c:if>
	        <c:if test="${redEnvelope.redEnvelopeType=='1'}">
	            <div class="form-group" id="randomPrice" style="display: none;">
	                <label class="control-label col-sm-2">最小金额</label>
	                <div class="col-sm-3" style="width:20%;">
		...
		<div class="form-group" style="background: #fff;padding: 30px 0px;margin-bottom:20px;">
            <div class="col-sm-12">
              <div class="navbar-btn">
	            ...
	            <input type="hidden" id="amountAllocated" value="${redEnvelopePrice}" />
              </div>
            </div>
        </div>
		...
		<div class="form-group">
			<div class="col-sm-12">
				<div class="navbar-btn">
					...
					<input type="button" value="取消" class="redBtnRad col-sm-offset-1"
						id="cancel" />
				</div>
			</div>
		</div>
							...
							function() {
								...
								if ($('input[name="redEnvelopeType"]:checked')
									...
									data = {
										...
										"amountAllocated" : $('#amountAllocated').val(),
										"redEnvelopeType" : $('input[name="redEnvelopeType"]:checked').val()
									}
								}
								if ($('input[name="redEnvelopeType"]:checked')
										.val() == '1') {
								...
								$
										.ajax({
											type : "POST",
											url : '<spring:url value="/headquarters/redEnvelope/doEditRedEnvelope" />',
											data : data,
											dataType : "json",
											async : false,
